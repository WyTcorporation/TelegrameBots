...
from NewHouseBot.modules.Keyboards.TextButtonsKb import getButtons
from NewHouseBot.modules.other.Request import Request
from NewHouseBot.modules.other.States import States


async def getName(message: Message, state: FSMContext, request: Request):
    await message.answer(
        'Привіт, Я помічник Кеша. Чим я можу Вам допомогти?',
        reply_markup=await getButtons(), disable_notification=True)
...
async def getNumber(message: Message):
    try:
